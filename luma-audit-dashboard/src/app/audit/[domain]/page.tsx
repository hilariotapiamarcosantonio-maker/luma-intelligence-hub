                ...
                <div className="relative bg-[#0a0a0a] border border-white/5 rounded-2xl p-8 shadow-2xl overflow-hidden">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        <div className="lg:col-span-4 space-y-8">
                            <div>
                                <h2 className="text-3xl font-bold text-white mb-1 truncate">{client_identity.company_name || domain}</h2>
                            ...
                            <div className="p-6 rounded-xl bg-black/40 border border-white/5">
                                ...
                                <span className={`text-6xl font-black ${isCritical ? 'text-red-500' : 'text-yellow-500'}`}>{pain_point_synthesis.authority_score}</span>
                                ...
                                <div className={`h-full ${isCritical ? 'bg-red-500' : 'bg-yellow-500'}`} style={{ width: `${pain_point_synthesis.authority_score}%` }}></div>
                                </div>
                            </div>
                            ...
                            <div className="p-6 rounded-xl bg-red-950/20 border border-red-900/30 relative overflow-hidden">
                                ...
                                <p className="text-3xl font-mono text-white mb-1">${lostRevenue.toLocaleString()} <span className="text-sm text-gray-500 font-sans">/mo</span></p>
                            ...
                            <div className="space-y-6">
                                <div className="bg-[#111] p-6 rounded-xl border border-white/5">
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        ...
                                        technical_audit.tech_stack.map((tech: string) => (
                                            <span key={tech} className="px-3 py-1 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded-md text-xs font-medium">{tech}</span>
                                        ))
                                    ...
                                    <div className="mt-4 pt-4 border-t border-white/5">
                                        ...
                                        {['Meta Pixel', 'Google Analytics', 'Google Tag Manager'].map(tracker => {
                                            ...
                                            <div key={tracker} className="flex items-center justify-between text-sm">
                                                ...
                                                {isActive ? <div className="w-2 h-2 rounded-full bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.6)]"></div> : <div className="w-2 h-2 rounded-full bg-red-500/50"></div>}
                                            </div>
                                            )
                                        })}
                                        </div>
                                ...
                                <div className="bg-[#111] p-6 rounded-xl border border-white/5">
                                    ...
                                    <div className="grid grid-cols-2 gap-4">
                                        <div><p className="text-xs text-gray-500 mb-1">Speed Score</p><p className="text-2xl font-mono text-white">{technical_audit.pagespeed?.score || 'N/A'}</p></div>
                                        <div><p className="text-xs text-gray-500 mb-1">LCP (Carga visual)</p><p className="text-2xl font-mono text-white">{technical_audit.pagespeed?.lcp || 'N/A'}</p></div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                <div className="bg-[#111] p-6 rounded-xl border border-white/5">
                                    ...
                                    <div className="space-y-3">
                                        {marketing_intelligence.social_links?.length === 0 && marketing_intelligence.broken_links?.length === 0 && <p className="text-sm text-gray-500 italic">No social footprints detected.</p>}
